...
_G.scope_find_files = function()
  require('telescope.builtin').find_files({
    search_dirs = scopes.get_current_dirs()
  })
end
_G.scope_live_grep = function()
...
wk.register({ -- Normal Mode Keybindings
    ...
    d = {
        b = {
            ...
            B = { function() require('dap').set_breakpoint(vim.fn.input('Condition: '), nil, nil) end, "DAP Set Conditional Breakpoint" },
            p = { function() require('dap').set_breakpoint(nil, nil, vim.fn.input('Log point message: ')) end, "DAP Log at breakpoint" },
        },
        c = { function() require('dap').continue() end, "DAP Continue" },
        n = { function() require('dap').step_over() end, "DAP Next" },
        s = { function() require('dap').step_into() end, "DAP Step" },
        f = { function() require('dap').step_out() end, "DAP Fin" },
        l = { function() require('dap').run_last() end, "DAP Run Last" },
        t = { function() require('dap').terminate() end, "DAP Terminate" },
        u = {
            name = "DAP-UI",
            u = { function() require("dapui").toggle() end, "Toggle" },
            h = { function() require('dap.ui.widgets').hover() end, "Hover" },
            p = { function() require('dap.ui.widgets').preview() end, "Preview" },
            f = { function() local widgets = require('dap.ui.widgets') widgets.centered_float(widgets.frames) end, "Float Frames" },
            s = { function() local widgets = require('dap.ui.widgets') widgets.centered_float(widgets.scopes) end, "Float Scopes" },
        },
        r = {
            name = "REPL",
            r = { function() require('dap').repl.toggle() end, "Toggle" },
            o = { function() require('dap').repl.open() end, "Open" },
        },
    },
    ...
    n = {
        name = "Notifications",
        c = { function() require('mini.notify').clear() end, "Clear Notifications" },
        h = { function() require('mini.notify').show_history() end, "Clear Notifications" },
    },
    ...
    s = {
        ...
        c = {
            name = "Scope",
            c = { function() require("neoscopes").select() end, "Select Scope" },
            -- f = { function() scope_find_files() end, "Find Files in scope" },
            f = { function() local search_dirs = scopes.get_current_dirs() vim.call("fzf#vim#files", search_dirs[1]) end, "Find Files in scope" },
            g = { function() scope_live_grep() end, "Live Grep in scope" },
        }
    },
